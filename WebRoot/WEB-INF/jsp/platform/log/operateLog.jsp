...
<script type="text/javascript">
	...
	$().ready(function()
	{
		App.loadActions("OperateLog", loadActionsBack);
	...
	function doSearch()
	{
		...
		$table = $("#tbList").bootstrapTable("destroy").bootstrapTable({
			 url: "sysLog/operateList.do?fmtm=" + fmtm +"&totm=" + totm + "&actionType=" + actionType,
			 pagination: true,
	...
	function doExport()
	{
		...
		$.get("sysLog/listXls.do?fmtm=" + fmtm +"&totm=" + totm + "&actionType=" + actionType + "&logType=1", function(data, status)
		{
			$xlsTable.bootstrapTable("load", data);
			
			App.handleExport($("#xls-table"), config.title_xls_operate_log_platform);
