			...
			<div class="control-group">
				...
				<div class="controls">
					<div class="input-append">
						<input type="text" placeholder="登录名或姓名" class="m-wrap" id="param" />
...
<div class="row-fluid">
	<div class="span12 portlet box blue">
		<div class="portlet-title">
			<div class="caption">
				<i class="icon-globe"></i><span class="titleExcel">企业出入库用户列表</span>
			</div>
		</div>
		...
		<div id="formEdit" class="modal hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">
			...
			<div class="modal-body">
				<form action="#" id="fmUserEdit" class="form-horizontal">
					...
					<input type="hidden" value="${C_USR.company_id }" id="company_id">
					<div class="control-group">
						<label class="control-label">
							登录名
...
<script type="text/javascript">
	...
	$().ready(function()
	{
		App.loadActions("CompanyPDAUser", loadActionsBack);
	...
	function doSearch()
	{
		...
		$table = $("#tbList").bootstrapTable("destroy").bootstrapTable({
			 url: "account/padUserList.do?searchParam=" + $("#param").val(),
			 pagination: true,
	...
	function doEdit()
	{
		...
		$.get(url, function(data, status)
		{
			...
			{
				$("input[name='pda_action'][value='" + data.actions[i] + "']").attr("checked", "checked");
			}
		});
	...
	function doSave()
	{
		...
		$("input[name='pda_action']:checked").each(function(index)
		{
		...
		$.ajax({
	        url: "account/pdaUserSave.do?rlId=8",
	        data: formParam,
	        method: "POST",
	        traditional: true
	    }).done(function (data) {
	...
	function loadPDAActions()
	{
		$.get("account/loadPdaActions.do", function(data, status)
		{
			$("#role_checkbox").empty();
			var str = "";
			for(var i = 0; i < data.length; i++)
			{
				...
				str += "<input type='checkbox' name='pda_action' value='" + data[i]["dctcd"] + "' style='margin-left: 0px;' />&ensp;" + data[i]["tpnm"];
				str += "</label> ";
	...
	function doExport()
	{
		App.executeExport("account/exportPDAUserList.do", {searchParam: $("#param").val(), title: config.title_xls_PDAUser_company});
	}
</script>
