...
{
	private static final String[] NoFilters = new String[] { "index.html", "login.jsp", "doLogin.do", Constants.LOGIN_URL, Constants.LOGOUT_URL , "signin.do", "sendMsg.do", "forget_password.do"};
	
	Logger logger = Logger.getLogger(CustomInterceptor.class);
	/**
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception
	{
		...
		{
			//logger.error("****" + user.getLgnNm() + "*" + request.getRequestURI());
		}
		...
		if (beFilter)
		{
			if (user == null // 需要用户验证的系统功能
					|| (user != null && "".equals(user.getCompany_id()) && "".equals(user.getSupplier_id()) 
							&& "".equals(user.getDealer_id()) && !ctrl.equals("platform")) // 平台用户企图访问其他视图
					|| (user != null && !"".equals(user.getCompany_id()) && "".equals(user.getSupplier_id()) 
							&& "".equals(user.getDealer_id()) && !ctrl.equals("admin")) // 企业用户企图访问其他视图
					|| (user != null && "".equals(user.getCompany_id()) && !"".equals(user.getSupplier_id()) 
							&& "".equals(user.getDealer_id()) && !ctrl.equals("supplier")) // 供应商用户企图访问其他视图
					|| (user != null && "".equals(user.getCompany_id()) && "".equals(user.getSupplier_id()) 
							&& !"".equals(user.getDealer_id()) && !ctrl.equals("dealer")) // 经销商用户企图访问其他视图
				)
			{
