...
public class SystemLogAspect
{
	...
	private void doInsertLog(JoinPoint joinPoint, String result)
	{
		...
		try
		{
			...
			log.setOperator(user == null ? "-" : user.getLgnNm());
			log.setIp(request.getRemoteAddr());
			log.setBrowser(request.getHeader("User-Agent"));
			log.setAction_id(joinPoint.getTarget().getClass().getName() + ":" + joinPoint.getSignature().getName() + "()");
			log.setFunction_id(request.getHeader("Referer").split(request.getContextPath())[1]);
			log.setAction_type(getServiceMthodDescription(joinPoint));
			log.setContent(params.substring(0, (params.length() >= 1999 ? 1999 : params.length())));
			log.setResult(result.substring(0, (result.length() >= 1999 ? 1999 : result.length())));
			log.setStatus(1);
			//保存数据库  
