...
<div class="un-print">
	...
	<input type="hidden" id="isOil" value="${isOil}" />
	...
	function showContent(data)
	{
		...
		else
		{
			...
			while( i < len)
			{
				...
				if (pkgLevel == 1 && (isSingle != 1 || isOil == 1) && i < len) {
					storeQr[i] = data[i]["ID"];
					if (isOil == 1) {
						str = getOilTableHtml(str, i, productName, data[i]["SERIALNO"], len, data[i]["SAPNO"], data[i++]["SPECNO"]);
					} else {
				...
				if (isOil == 1) {
					str += "<tr>";
					str += "<td colspan='2' style='text-align: center;'>无锡威孚高科技集团股份有限公司</td>";
					if (pkgLevel == 1 && (isSingle != 1 || isOil) && i < len) {
						str += "<td colspan='2' style='text-align: center;'>无锡威孚高科技集团股份有限公司</td>";
					}
					str += "</tr>";
				}
